...
4. This is where things get hazy...
    ...
    - To this end, one option might? be to configure the Express Dockerfile to map a volume (`$(pwd)/build:/express/build`) with the host which contains the React assets — then with Express you could use something like: `.use(express.static(__dirname + '/build'))`
